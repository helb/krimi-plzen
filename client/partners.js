Template.partners.helpers({
	partnerSlideshowStyle: function() {
		...
		for (i = 0; i < partnerCount; i++) {
			...
			fakebgAnimation += percents + "% {left: " + (partnerCount - i) * -5.3 + "rem} ";
			fakebgAnimation += percents + fTime + "% {left: " + (partnerCount - i) * -5.3 + "rem} ";
		}
		...
		var fakebgStyle = ".partner-label-fakebg{animation: fakebgSlide " + partnerCount * 8 +
			"s ease-in-out infinite; left: " + partnerCount * -5.3 + "rem}";
