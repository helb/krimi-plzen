document.addEventListener("DOMContentLoaded", function (event) {
    ...
    function createGallery(thumbnails, galleryIndex) {
        ...
        Array.from(thumbnails.getElementsByTagName("a")).forEach((thumb, index) => {
            ...
            thumb.addEventListener("click", function (event) {
                ...
                galleryControls.querySelectorAll("a").forEach(link => link.classList.remove("current"));
                Array.from(
                ...
                if (!window.sliders["slider" + galleryIndex]) {
                    window.sliders["slider" + galleryIndex] = new Siema({
                        ...
                        onChange: function() {
                            galleryControls.querySelectorAll("a").forEach(link => link.classList.remove("current"));
                            let current = window.sliders["slider" + galleryIndex].currentSlide;
                            galleryControls.querySelectorAll("a")[current].classList.add("current");
                        }
                    });
                }
                ...
                galleryControls.querySelectorAll("a")[index].classList.add("current");

                galleryControls.querySelectorAll("a").forEach((link, controlindex) => link.addEventListener("click", function (event) {
                    event.preventDefault();
                    event.stopPropagation();
                    window.sliders["slider" + galleryIndex].goTo(controlindex);
                    galleryControls.querySelectorAll("a").forEach(link => link.classList.remove("current"));
                    galleryControls.querySelectorAll("a")[controlindex].classList.add("current");
                }));
            });
        });
    ...
    Array.from(document.querySelectorAll(".article-content .gallery")).forEach((gallery, galleryIndex) => {
