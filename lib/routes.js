...
Router.map(function () {
  this.route('newArticles', {
    ...
    if (Meteor.isClient) {
        ...
        return listSubs.subscribe('newArticlesLatest', Session.get("currentLimit"));
      }
    },
    cache: true,
    fastRender: true,
  ...
  this.route('archiv', {
	...
	var one = IRLibLoader.load('/js/better-dom.js');
      if(one.ready()){
        var two = IRLibLoader.load('/js/better-i18n-plugin.js');
      if(two.ready()){
        var three = IRLibLoader.load('/js/better-dateinput-polyfill.js');
      if(three.ready()){
      var four = IRLibLoader.load('/js/better-dateinput-polyfill.cs.js');
        if(four.ready()){
          this.next();
        }
        }
